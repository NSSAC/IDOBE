...
class FixedModelVanillaTransformerProcessor:
    ...
    def create_fixed_model(self, df_train, df_test, test_ids, h, freq, level=[]):
        ...
        model_cls = AutoVanillaTransformer
        if not level:
            nf = NeuralForecast(models=[model_cls(h=h, backend="optuna", config=self.config)], freq=freq)
        else:
            nf = NeuralForecast(models=[model_cls(h=h, backend="optuna", loss=MQLoss(level=level), config=self.config)], freq=freq)
        ...
        for uid in tqdm(test_ids, desc="Forecasting with AutoVanillaTransformer"):
    ...
    def create_display_df(self):
        ...
        for i, forecast_df in enumerate(tqdm(self.forecasts, desc="Building display_df")):
            ...
            for col in forecast_df.columns:
                ...
                elif col in ["AutoVanillaTransformer", "AutoVanillaTransformer-median"]:
                    quantile = 0.5
                else:
                    continue
...
processor = FixedModelVanillaTransformerProcessor()
...
processor.display_df.to_csv('../../output/AutoVanillaTransformer.csv',index=None)
# processor.forecasts
...
wis_table.to_csv('../../eval/WIS_AutoVanillaTransformer.csv',index=None)
