...
class FixedModelInformerProcessor:
    ...
    def create_fixed_model(self, df_train, df_test, test_ids, h, freq, level=[]):
        ...
        if not level:
            nf = NeuralForecast(models=[AutoInformer(h=h, backend="optuna", config=self.config)], freq=freq)
        else:
            nf = NeuralForecast(models=[AutoInformer(h=h, backend="optuna", loss=MQLoss(level=level), config=self.config)], freq=freq)
        ...
        for uid in tqdm(test_ids, desc="Forecasting with AutoInformer"):
        ...
        for i, forecast_df in enumerate(tqdm(self.forecasts, desc="Building display_df")):
            ...
            for col in forecast_df.columns:
                ...
                if "lo" in col:
                    ...
                    alpha = 1 - (number / 100)
                    quantile = 1 - (alpha / 2)
                elif col in ["AutoInformer", "AutoInformer-median"]:
                    quantile = 0.5
                else:
                    continue
...
processor = FixedModelInformerProcessor()
...
processor.display_df.Unique_id.unique()


# In[18]:


processor.display_df.to_csv('../../output/forecasts_AutoInformer.csv')
...
wis_table.to_csv('../../output/forecasts_WIS_AutoInformer.csv',index=None)


# In[17]:


wis_table.to_csv('../../eval/WIS_AutoInformer.csv')
